...
namespace LeakTestSystem.Services.MES
{
    public class MES_Service
    {
        ...
        public static bool UploadTestRecords(string ReelNo, List<string> listData, ref string message)
        {
            ...
            {
                //上传测试记录
                string data = $"{Sys_User.username};{ReelNo};";
                for (int i = 0; i < listData.Count; i++)
                {
                    data += listData[i] + ";";
                }
                var sp_data = data;
                //sysLog.WriteLog("UploadTestRecords", data);
        ...
        {
            string sp_data = $"{Sys_User.username};{serial_Number};{test_item};";
        ...
        public static bool GetProductionModelName(string SerialNumber, ref string message)
        {
            ...
            catch (Exception ex)
            {
                ...
                //sysLog.WriteLog("MESCheck", ex.Message); ;
                return false;
            }
        }
        ...
        {
            SerialNumber = $"{Sys_User.username};{SerialNumber};OK;";
        ...
        {
            SerialNumber = $"{Sys_User.username};{SerialNumber};NG;{NG_Code};";
            var res = CallMesCommand(3, SerialNumber);
            if (res.receive.IndexOf("OK") > -1)
            {
    ...
    public class MesCommand
    {
        ...
        {
            return Path.Combine("SajetConnect.dll");
        }
    }
}