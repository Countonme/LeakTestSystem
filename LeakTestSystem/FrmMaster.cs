...
namespace LeakTestSystem
{
    public partial class FrmMaster : UIForm
    {
        ...
        {
            int index = _config.GetChannelIndexByComName(_config, result.comName);

            ShowLogs(
                $"ComName:{result.comName} ComIndex:{index} TestCount:{resultList.Count} Total:{_config.GetEnableChannelCount(_config)}",
                Color.DarkGreen);
            ...
            InitUIDisplay(
                status == "OK" ? PASS : FAIL,
                index,
                status == "OK" ? Color.Green : Color.Red);

            List<TestResult> batch = null;
        ...
        private void ProcessBatch(List<TestResult> batch, string message)
        {
            bool hasNg = batch.Any(x => x.testResult == "NG");
            ...
            UploadMes(batch, uuid, message);
        }

        private void UploadMes(
    List<TestResult> batch,
    string uuid,
    string message)
        {
            ShowLogs(
                $"[MES开始] UUID:{uuid} 总数:{batch.Count}",
                Color.DarkBlue);
        ...
        private void UploadSingleResult(
    string uuid,
    int index,
    int total,
    TestResult result,
    string message)
        ...
        {
            $"PressureValue:{result.PressureValue}:{status}",
            $"LeakValue:{result.LeakValue}:{status}"
        };

                if (switchMesNgLock.Active)
                {
                    UploadRecord(result, items, uuid, index, total, message);

                    CrossNg(result, uuid, index, total, ref message);
                ...
                {
                    ShowLogs(
                        $"[NG跳过MES] SN:{result.serialNumber}",
                        Color.Orange);
                ...
                CrossPass(result, uuid, index, total, ref message);
            }
            catch (Exception ex)
            {
        ...
        private void UploadRecord(TestResult result, List<string> items, string uuid, int index, int total, string message)
        {
            ShowLogs(
                $"[上传测试记录] {index}/{total} SN:{result.serialNumber}",
                Color.DarkBlue);

            if (!MES_Service.UploadTestRecords(
                    result.serialNumber,
                    items,
                    ref message))
            {
            ...
            ShowLogs(
                $"[上传成功] SN:{result.serialNumber}",
                Color.Green);
        }

        private void CrossPass(TestResult result, string uuid, int index, int total, ref string message)
        {
            ShowLogs(
                $"[开始过站] UUID:{uuid} {index}/{total} SN:{result.serialNumber}",
                Color.DarkBlue);
            ...
            if (!MES_Service.SerialNumberCorssingStationPass(result.serialNumber, ref message))
            {
                ...
                ShowLogs(
                    $"[过站失败] UUID:{uuid} {index}/{total} SN:{result.serialNumber} Message:{message}",
                    Color.Red);

                this.ShowErrorDialog($"SN：{result.serialNumber}\r\nMES过站失败：{message}");

                throw new Exception(message);
            }

            ShowLogs(
                $"[过站成功] UUID:{uuid} {index}/{total} SN:{result.serialNumber}",
                Color.Green);
        }

        private void CrossNg(TestResult result, string uuid, int index, int total, ref string message)
        {
            ShowLogs(
                $"[开始NG过站] UUID:{uuid} {index}/{total} SN:{result.serialNumber}",
                Color.DarkBlue);

            if (!MES_Service.SerialNumberCorssingStationFail(
                    result.serialNumber,
                    _config.ngCode,
                    ref message))
            {
                ...
                ShowLogs($"[NG过站失败] UUID:{uuid} {index}/{total} SN:{result.serialNumber} Message:{message}",
                    Color.Red);
            ...
            ShowLogs(
                $"[NG过站成功] UUID:{uuid} {index}/{total} SN:{result.serialNumber}",
                Color.Green);
        }
